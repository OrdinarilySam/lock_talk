...
mod tests {
    ...
    fn decrypt_128bit_block_pad() {
        ...
        let expected: Vec<u8> = vec![b'A'; 16];

        assert_eq!(result, expected);
    }

    #[test]
    fn decrypt_192bit_1pad() {
        let key: Vec<u8> = vec![b'A'; 24];
        let input: Vec<u8> = vec![
            0xE6, 0xA4, 0xA5, 0x9B, 
            0xF8, 0xF9, 0x60, 0x3B, 
